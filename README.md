# Gradescope Due Dates Dashboard

Gradescope Due Dates Dashboard is a Manifest V3 Chrome extension that opens a side panel, scans the logged-in user's Gradescope course pages, caches assignments in `chrome.storage.local`, and shows them in a compact dark planner layout.

## What it does

- Opens from the Chrome toolbar into a side panel.
- Collects assignments by visiting real Gradescope course pages instead of relying only on the homepage.
- Normalizes assignment fields such as title, course name, due date, status, points, and direct URL.
- Groups assignments into planner-style sections like Due Today, Due Tomorrow, Due This Week, Later, Overdue, and No Due Date.
- Caches the latest scrape in local extension storage and refreshes again when Gradescope is revisited.
...
- `sidepanel.css`: dark dashboard styling.
...
4. Select this folder: `/Users/aweso/Documents/New project`
...
5. Press **Refresh** if the panel does not begin scanning automatically.
6. Confirm that:
   ...
   - clicking an assignment opens the correct Gradescope page
...
- Some Gradescope pages may vary by course type or account state. The scraper uses fallback selectors and partial-success handling so one course failure does not block the rest.

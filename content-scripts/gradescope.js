(() => {
  ...
  const ASSIGNMENT_TARGET_SELECTOR = [
    "a[href*='/assignments/']",
    "[data-href*='/assignments/']",
  ...
  function collectCourseLinks() {
    ...
    for (const anchor of anchors) {
      ...
      const isCourseEntry = isCourseHomeUrl(url);
      const inCourseCard = Boolean(anchor.closest(".courseBox, .courseListItem, .courseList-item, [class*='courseBox'], [class*='courseList']"));

      if (!courseId || (!isCourseEntry && !inCourseCard) || courseMap.has(courseId)) {
        continue;
      }

      const courseUrl = `${window.location.origin}/courses/${courseId}`;
      const courseName = findCourseNameFromAnchor(anchor);
  ...
  function collectAssignmentListUrls(courseUrl) {
    ...
    for (const anchor of anchors) {
      const url = normalizeAbsoluteUrl(anchor.getAttribute("href"));
      if (!shouldVisitCoursePage(url, courseId, anchor)) {
        continue;
      }
  ...
  function collectAssignments(courseName, courseUrl) {
    const assignmentLinks = Array.from(document.querySelectorAll(ASSIGNMENT_TARGET_SELECTOR));
    const seenUrls = new Set();
    const assignments = [];

    for (const link of assignmentLinks) {
      const url = normalizeAbsoluteUrl(extractUrlFromElement(link));
  ...
  function findAssignmentContainer(link) {
    return link.closest("tr, li, article, section, [role='row'], .card, .assignment, .submission, .table--row, .table-row") || link.parentElement;
  }
  ...
  function isCourseHomeUrl(url) {
    return /\/courses\/\d+(?:[/?#]|$)/.test(String(url || "")) && !extractAssignmentId(url);
  }

  function findCourseNameFromAnchor(anchor) {
    const card = anchor.closest(".courseBox, .courseListItem, .courseList-item, article, li, section, div");
    const candidates = [
      anchor.getAttribute("aria-label"),
      anchor.getAttribute("title"),
      anchor.dataset.courseName,
      findFirstText(card || document, [
        ".courseBox--shortname",
        ".courseBox--name",
        ".courseListItem-name",
        ".courseListItem-title",
        "[class*='courseBox'][class*='name']",
  ...
  function shouldVisitCoursePage(url, courseId, anchor) {
    if (!isCourseRelatedUrl(url, courseId) || extractAssignmentId(url)) {
      return false;
    }
    ...
    try {
      ...
      const text = cleanText(anchor ? anchor.textContent : "").toLowerCase();
      const rel = cleanText(anchor ? anchor.getAttribute("rel") : "").toLowerCase();
      const inPagination = Boolean(anchor && anchor.closest(".pagination, nav[aria-label*='Pagination'], [class*='pagination']"));

      if (path === `/courses/${courseId}` || path.startsWith(`/courses/${courseId}/assignments`) || path.startsWith(`/courses/${courseId}/submissions`)) {
        return true;
      }

      if (inPagination || rel.includes("next") || rel.includes("prev") || /^\d+$/.test(text)) {
        return true;
      }

      if (/(?:\?|&)(?:page|tab|status|view|filter|sort)=/i.test(search)) {
        return true;
      }

      return COURSE_PAGE_TEXT_HINTS.some((hint) => text.includes(hint));
    } catch (error) {
      return false;
    }
  }

  function isCourseRelatedUrl(url, courseId) {
    return extractCourseId(url) === String(courseId);
  }
